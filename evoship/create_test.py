...
def create_linear_sweep(part, solid_id, sweep_param, config):
    return part.CreateLinearSweep(
        str(solid_id), config.operation_type, sweep_param, False
    )
...
def main():
    ...
    cylinder_feature_id = create_linear_sweep(part, solid_id, sweep_param, config)
    blank_element(part, circle_id)

    print(
        f"半径{config.radius_mm}mmの円を長さ{config.height_mm}mmで押し出した円柱を生成しました"
    )
