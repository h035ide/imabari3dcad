...
class HybridRetriever(BaseRetriever):
        ...
        if self.config.enable_fulltext:
            ...
            if self.config.elasticsearch_config:
                try:
                    ...
                    logging.warning(
                        "Failed to initialize Elasticsearch retriever: %s", exc
                    )
    ...
    def search(self, context: QueryContext) -> List[SearchResult]:
        ...
        from .parallel_retriever import ParallelRetriever  # type: ignore[reportMissingImports]
        ...
        if self._collection is None:
            try:
                ...
                if self.persist_directory:
                    settings = Settings(
                        persist_directory=self.persist_directory, is_persistent=True
                    )
