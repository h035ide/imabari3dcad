...
def validate_graph_data(nodes: List[Dict[str, Any]], relationships: List[Dict[str, Any]]) -> None:
    """
    グラフデータの妥当性をチェックする
...
def validate_relationship(relationship: GraphRelationship, valid_node_ids: set = None) -> None:
    """
    ...
    if not isinstance(relationship.type, RelationshipType):
        raise ValidationError(f"無効なリレーションシップタイプです: {relationship.type}")
    ...
    if valid_node_ids:
        ...
        if relationship.target not in valid_node_ids:
            raise ValidationError(f"存在しないターゲットノードID: {relationship.target}")
...
def validate_relationship_dict(relationship: Dict[str, Any], valid_node_ids: set = None) -> None:
    """
    ...
    for key in required_keys:
        if key not in relationship:
            raise ValidationError(f"リレーションシップに必須フィールドが不足しています: {key}")
