    ...
    def parse_api_specs(
        ...
        for file_name, content in api_files:
            try:
                ...
                logger.info(
                    f"解析完了: {file_name} - ノード={len(nodes)}件, リレーション={len(rels)}件"
                )
        ...
        # データの統合とクリーニング
        merged_nodes, merged_relationships = self._merge_graph_data(
            all_nodes, all_relationships
        )

        # トリプル形式に変換
        triples, node_props = self._convert_to_triples(
            merged_nodes, merged_relationships
        )

        logger.info(
            f"API仕様書解析完了: ノード={len(merged_nodes)}件, リレーション={len(merged_relationships)}件"
        )
        return triples, node_props

    def parse_datatype_descriptions(
        self, api_arg_candidates: List[Path]
    ) -> Dict[str, str]:
        """
        ...
        for node in nodes:
            ...
            if node_id:
                if node_id in merged_nodes_dict:
                    # 既存のノードプロパティを更新
                    existing_props = merged_nodes_dict[node_id].setdefault(
                        "properties", {}
                    )
    ...
    def _convert_to_triples(
        ...
        for rel in relationships:
            ...
            if source_id not in node_type_map or target_id not in node_type_map:
                logger.warning(
                    f"存在しないノードIDのリレーションをスキップ: {source_id} -> {target_id}"
                )
                continue
