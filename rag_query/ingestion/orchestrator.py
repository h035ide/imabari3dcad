...
class IngestionOrchestrator:
    ...
    def run_ingestion(self, config: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Dict[str, Any]]]:
        """
        データ取り込み処理を実行する
    ...
    def _process_api_specs(self, config: Dict[str, Any], errors: List[str]) -> Tuple[List[Dict[str, Any]], Dict[str, Dict[str, Any]]]:
        """
        API仕様書を処理する
        ...
        try:
            ...
            type_descriptions = self.api_parser.parse_datatype_descriptions(api_arg_candidates)
    ...
    def _process_scripts(self, config: Dict[str, Any], errors: List[str]) -> Tuple[List[Dict[str, Any]], Dict[str, Dict[str, Any]]]:
        """
        スクリプト例を処理する
        ...
        try:
            ...
            for script_path, script_text in script_files:
                try:
                    triples, node_props = self.script_analyzer.analyze_script(script_path, script_text)
    ...
    def _add_datatype_descriptions(self, node_props: Dict[str, Dict[str, Any]], 
                                  type_descriptions: Dict[str, str]) -> None:
        """
        データ型ノードに説明を追加する
        ...
        for node_id, node_data in node_props.items():
            if (node_data.get("type") == "DataType" and 
                node_data.get("properties", {}).get("name") in type_descriptions):
