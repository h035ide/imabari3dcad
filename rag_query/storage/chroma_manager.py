...
class ChromaManager:
    ...
    def build_vectorstore(self, graph_docs: List[GraphDocument], 
                         save_debug_data: bool = True) -> None:
        """
        ...
        try:
            ...
            logger.info(f"ChromaDBベクトルストア構築完了: {len(documents)}件のドキュメント")
    ...
    def load_vectorstore(self) -> Chroma:
        ...
        try:
            if not self.persist_directory.exists():
                raise StorageError(f"ChromaDBディレクトリが存在しません: {self.persist_directory}")
            ...
            logger.info(f"ChromaDBベクトルストアを読み込みました: {self.persist_directory}")
            return vectorstore
    ...
    def _clear_persist_directory(self) -> None:
        ...
        if self.persist_directory.exists():
            ...
            logger.info(f"既存のChromaDBディレクトリを削除しました: {self.persist_directory}")
