...
class GraphBuilder:
    ...
    def build_graph_documents(self, triples: List[Dict[str, Any]], 
                            node_props: Dict[str, Dict[str, Any]]) -> List[GraphDocument]:
        """
        ...
        try:
            ...
            for node_id, meta in node_props.items():
                if node_id in node_map:
                    ...
                    node_map[node_id] = Node(id=node_id, type=node_type, properties=properties)
            ...
            logger.info(f"GraphDocument構築完了: ノード={len(node_map)}件, リレーション={len(relationships)}件")
            return [graph_doc]
