...
2.  **API発見:** 次に、抽出した「意図」を基に`hybrid_graph_knowledge_search`ツールを使い、最も関連性の高いAPIとその周辺情報（シグネチャ、所属クラス、呼び出し元など）を発見します。
3.  **情報検証と比較:** 発見したAPIが必要とするパラメータと、ユーザーが提供したパラメータを比較します。APIの所属クラスや呼び出し元の情報も参考に、ユーザーの要求に最も合致するAPIか判断してください。
4.  **対話による情報補完:**
