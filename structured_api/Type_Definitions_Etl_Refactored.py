...
class TypeDefinitionProcessor:
    ...
    def __init__(self, node_builder: NodeBuilder, relation_builder: RelationBuilder, config: GraphBuilderConfig):
        self.node_builder = node_builder
        self.relation_builder = relation_builder
        self.config = config
        self.metadata_processor = MetadataProcessor()
    
    def process_type_definition(self, td: Dict[str, Any], index: int) -> Tuple[List[Node], List[Relation]]:
        """単一の型定義を処理する"""
        nodes = []
        relations = []
    ...
    def _create_type_definition_node(self, td: Dict[str, Any], name: str, index: int) -> Tuple[Node, List[Relation]]:
        ...
        primary_text = self.metadata_processor.build_text(
            self.node_builder._to_str(description), self.node_builder._to_str(source_text)
        )
        ...
        relation = self.relation_builder.create_relation(collection_uid, "HAS_TYPE_DEFINITION", td_uid)
    ...
    def _process_canonical_type(self, td: Dict[str, Any], name: str) -> List[Node]:
        ...
        if isinstance(python_meta, dict) and python_meta.get("name"):
            ...
            py_description = self.node_builder._to_str(python_meta.get("description", ""))
    ...
    def _process_variants(self, td: Dict[str, Any], name: str) -> Tuple[List[Node], List[Relation]]:
        """バリアントを処理する"""
        nodes = []
        relations = []
        ...
        for origin_key in ("one_of", "variants"):
            ...
            for v_idx, entry in enumerate(variant_entries):
                variant_nodes, variant_relations = self._process_single_variant(entry, name, v_idx, origin_key)
                nodes.extend(variant_nodes)
                
                # バリアントからTypeDefinitionへのリレーションを追加
                if variant_nodes:
                    variant_uid = variant_nodes[0].uid  # 最初のノードがメインのVariantノード
    ...
    def _process_single_variant(
        ...
        if metadata and metadata.get("pattern"):
            pattern_nodes, pattern_relations = self._process_pattern(metadata["pattern"], variant_uid)
            nodes.extend(pattern_nodes)
            relations.extend(pattern_relations)
        
        # 制約の処理
        if constraints:
            constraint_nodes, constraint_relations = self._process_constraints(constraints, variant_uid)
            nodes.extend(constraint_nodes)
            relations.extend(constraint_relations)
        
        return nodes, relations
    
    def _process_pattern(self, pattern: Any, variant_uid: str) -> Tuple[List[Node], List[Relation]]:
        """パターンを処理する"""
        nodes = []
        relations = []
        ...
        if pattern_str:
            ...
            pattern_type = "regex" if self.metadata_processor.is_regex_pattern(pattern_str) else "literal"
            ...
            # パターンからバリアントへのリレーション
            relation = self.relation_builder.create_relation(variant_uid, "HAS_PATTERN", pattern_uid)
            relations.append(relation)
        
        return nodes, relations
    
    def _process_constraints(self, constraints: Dict[str, Any], variant_uid: str) -> Tuple[List[Node], List[Relation]]:
        """制約を処理する"""
        nodes = []
        relations = []
        ...
        # 制約からバリアントへのリレーション
        relation = self.relation_builder.create_relation(variant_uid, "HAS_CONSTRAINT", constraint_uid)
        relations.append(relation)
        ...
        for prop_key, prop_value in constraints.items():
            if prop_key not in {"kind", "notes", "length", "schema"}:
                ...
                # プロパティから制約へのリレーション
                prop_relation = self.relation_builder.create_relation(constraint_uid, "HAS_PROPERTY", prop_uid)
                relations.append(prop_relation)
        
        # スキーマアイテムの処理
        schema_items = constraints.get("schema")
        if isinstance(schema_items, list):
            schema_nodes, schema_relations = self._process_schema_items(schema_items, constraint_uid)
            nodes.extend(schema_nodes)
            relations.extend(schema_relations)
        
        return nodes, relations
    
    def _process_schema_items(self, schema_items: List[Any], constraint_uid: str) -> Tuple[List[Node], List[Relation]]:
        """スキーマアイテムを処理する"""
        nodes = []
        relations = []
        ...
        for s_idx, raw_item in enumerate(schema_items):
            ...
            schema_relation = self.relation_builder.create_relation(constraint_uid, "HAS_SCHEMA_ITEM", schema_uid)
            ...
            if isinstance(raw_item, str):
                ...
                if self.metadata_processor.is_regex_pattern(raw_item):
                    ...
                    regex_relation = self.relation_builder.create_relation(schema_uid, "HAS_REGEX_PATTERN", regex_uid)
                    relations.append(regex_relation)
                # 列挙値の処理（|で区切られた値）
                elif "|" in raw_item:
                    enum_nodes, enum_relations = self._process_enumeration_values(raw_item, schema_uid)
                ...
                # 単一の固定値
                    ...
                    literal_relation = self.relation_builder.create_relation(schema_uid, "HAS_LITERAL_VALUE", literal_uid)
    ...
    def _process_enumeration_values(
        ...
        nodes = []
        relations = []
        for enum_chunk in [chunk.strip() for chunk in enum_string.split("|") if chunk.strip()]:
            ...
            enum_relation = self.relation_builder.create_relation(schema_uid, "ALLOWS_VALUE", enum_uid)
        ...
        if isinstance(examples, list):
            for ex_idx, ex in enumerate(examples):
                ...
                node = self.node_builder.create_node(
                    ...
                    position=ex_idx,
                    metadata_json=self.node_builder._safe_json(metadata) if metadata else None,
                )
                nodes.append(node)
...
class GraphBuilder:
    ...
    def build_graph_elements(self, definitions: List[Dict[str, Any]]) -> Tuple[List[Node], List[Relation]]:
        """グラフ要素を構築する"""
        nodes_map: Dict[str, Node] = {}
        relations: List[Relation] = []
        
        collection_uid = "type_definitions::collection"
        collection_node = self.node_builder.create_node("TypeDefinitions", collection_uid, name="type_definitions")
        nodes_map[collection_uid] = collection_node
        
        for index, td in enumerate(definitions):
            td_nodes, td_relations = self.type_processor.process_type_definition(td, index)
    ...
    def _create_additional_relations(self, td: Dict[str, Any], nodes: List[Node]) -> List[Relation]:
        """追加のリレーションを作成する"""
        relations = []
        name = (td.get("name") or td.get("type_name") or "").strip()
        if not name:
        ...
        canonical = (td.get("canonical_type") or "").strip()
        if canonical:
            relations.append(self.relation_builder.create_relation(
        ...
        if isinstance(aliases, list):
            for alias_idx, alias_value in enumerate(aliases):
                ...
                for origin_key in ("one_of", "variants"):
                    variant_entries = td.get(origin_key) or []
                    if isinstance(variant_entries, list) and alias_idx < len(variant_entries):
            ...
            if isinstance(variant_entries, list):
                for v_idx, entry in enumerate(variant_entries):
                    if isinstance(entry, dict):
                        ...
                        if value_kind and self.config.canonical_meta.get(self.node_builder._to_str(value_kind)):
                            identifier = (
                                entry.get("id")
                                or entry.get("variant")
                                or entry.get("value")
                            ...
                            variant_uid = f"variant::{name}::{identifier_str}"
                            relations.append(self.relation_builder.create_relation(
                                variant_uid, "CONSTRAINED_BY_CANONICAL_TYPE", f"canonical::{value_kind}"
                            ))
        ...
        if source_text or source_path:
            source_uid = f"source::{name}::{self.node_builder._normalize_uid(source_path or 'inline')}"
            relations.append(self.relation_builder.create_relation(td_uid, "CITED_FROM", source_uid))
