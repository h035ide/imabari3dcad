...
関数の仕様（引数の型、書式は[api_arg 1.md](./api_arg%201.md)を参照）
...
### スケッチ円弧を中心点と始終点を指定して作成

**メソッド:** `CreateSketchArc`
...
### スケッチＮＵＲＢＳ線を中心点を指定して作成

**メソッド:** `CreateSketchNURBSCurve`
...
### プロファイル要素を押し出してシート要素を作成

**メソッド:** `CreateLinearSweepSheet`
...
### シート要素の向き（表側、法線方向）を指定した方向に揃える

**メソッド:** `SheetAlignNormal`
...
### 指定したソリッド要素に別のソリッド要素形状を付加する

**メソッド:** `CreateOtherSolid`
...
### 指定したソリッド要素に押し出し形状を付加する

**メソッド:** `CreateLinearSweep`
...
### 船殻のブラケット要素のパラメータオブジェクトの作成

**メソッド:** `CreateBracketParam`

**返り値:** ブラケット要素のパラメータオブジェクト

**パラメータ:** なし

#### ブラケット要素のパラメータオブジェクトの属性
...
| `ElementGroup` | 要素グループ | 作成するソリッド要素を要素グループに入れる場合は要素グループを指定（空文字可） |
| `MaterialName` | 材料 | 作成するソリッド要素の材質名称（空文字可） |
| `BasePlane` | 平面 | 面指定の場合の基準平面 |
| `BasePlaneOffset` | 長さ | 基準平面のオフセット距離 |
| `BaseElement` | 要素 | 基準要素指定の場合の基準要素 |
...
### 船殻の条材ソリッド要素のパラメータオブジェクトの作成

**メソッド:** `CreateProfileParam`
...
| 属性名 | 型 | 説明 |
|--------|----|----|
| `DefinitionType` | 整数 | の作成方法指定 0:取付線指定 1:基準面指定 2:取付線＋指定方向線 3: 元要素指定 4:ホール指定 5:２点 6:ロンジ間 7:基準線指定 8:基準点と方向 9:基準要素 |
...
### ボディを指定したソリッドで削除することで分割する（ボディの区分けコマンド）

**メソッド:** `BodySeparateBySubSolids`
